...
def main():
    ...
    if arg_one == "--user":
        if len(sys.argv) < 4:
            print("usage: python3 chess_api_cli.py --user [username1] [username2]")
            exit()
        else:
            user1, user2 = sys.argv[2] , sys.argv[3]
            get_games(user1,user2)
...
def get_games(user1, user2):
    ...
    for user in users:
        ...
        for url in month_urls["archives"]:
            ...
            year = match.group(1)
            month = match.group(2)
            if year not in user_games:
                user_games[year] = []
            user_games[year].append({month:res})


        games_db[users] = user_games    
    ...
    with open(fname, "w") as f:
        json.dump(games_db,f)

    print(f"Saved data from {', '.join(user)} to {fname}")
